...
export function BNPLCalculator({ price }: BNPLCalculatorProps) {
  ...
  // Koko: 3 equal monthly installments, 0% interest
  const kokoInstallment = Math.ceil(price / 3)
    ...
    <Card className="border-border rounded-2xl overflow-hidden">
      <CardContent className="p-0">
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          <TabsList className="w-full grid grid-cols-2 rounded-none h-auto p-0 bg-muted/50">
            <TabsTrigger
              value="koko"
              className="rounded-none py-3 data-[state=active]:bg-[#FF6B00] data-[state=active]:text-white font-semibold transition-all"
            >
            ...
            <TabsTrigger
              value="mintpay"
              className="rounded-none py-3 data-[state=active]:bg-[#00D4AA] data-[state=active]:text-white font-semibold transition-all"
            >
              <span className="flex items-center gap-2">
                <svg viewBox="0 0 24 24" className="h-5 w-5" fill="currentColor">
          ...
          <TabsContent value="koko" className="m-0">
            <div className="p-4 bg-gradient-to-br from-[#FF6B00]/5 to-[#FF6B00]/10">
              <div className="flex items-start gap-3 mb-4">
                <div className="p-2 rounded-xl bg-[#FF6B00]/10">
                  <Calculator className="h-5 w-5 text-[#FF6B00]" />
                </div>
                <div>
                  <h4 className="font-semibold text-foreground">Pay with Koko</h4>
              ...
              <div className="space-y-3">
                <div className="flex justify-between items-center py-2 border-b border-[#FF6B00]/20">
                ...
                <div className="flex justify-between items-center py-3 bg-[#FF6B00]/10 rounded-xl px-3 -mx-1">
                  <span className="font-semibold text-foreground">Monthly Payment</span>
                  <span className="text-xl font-bold text-[#FF6B00]">
                    LKR {kokoInstallment.toLocaleString()}/mo
                  </span>
                </div>
              </div>

              <div className="mt-4 p-3 bg-white rounded-xl border border-[#FF6B00]/20">
          ...
          <TabsContent value="mintpay" className="m-0">
